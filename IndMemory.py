class IndMemory(Individuo):
      ...
      def train(self):
        ...
        epi= 500 # Número de episódios de treinamento
      ...
      def moveM(self,eatual,earredor,catual):
        e= 0.1
        ale= rd.uniform(0,1)
        rmax= np.argmax(self.Q[eatual]) # Qual ação dá a melhor recompensa
        ...
        elif len(X)>1 and ale<=e:
          idfut= rd.sample(range(len(X)),1)[0]# escolha aleatória do indice
          nova_coord= convert(X[idfut],Y[idfut],self.land.meta["transform"])
          self.coord.append([nova_coord[0],nova_coord[1]])

        else:
          nova_coord= convert(X,Y,self.land.meta["transform"])
          self.coord.append([nova_coord[0],nova_coord[1]])
